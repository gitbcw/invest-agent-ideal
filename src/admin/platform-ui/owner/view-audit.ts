...
function renderAuditScope(){
  ...
  document.getElementById('auditHelp').textContent=isPush?'推送审计查看主动推送入队正文、调度任务状态和关联的 scheduler LLM trace。':(isAutomation?'自动化任务查看每次运行的状态、失败分类、重试属性，以及该次运行关联的 LLM 任务输入与模型输出。':'对话审计查看微信/Web 用户消息进入 Mastra runtime 后的原始回复、清洗回复和入站提示（自动化任务执行不在此列）。');
}
...
function renderAutomationItem(item){const statusKind=item.status==='succeeded'?'ok':(item.status==='failed'?'warn':'gray');const meta=[['user',item.userId],['origin',item.origin],['attempt',item.attempt],['run',item.runId],item.agentModel?['model',item.agentModel]:null,item.deliveryStatus?['delivery',item.deliveryStatus]:null].filter(Boolean);const duration=item.startedAt&&item.finishedAt?Math.max(0,new Date(item.finishedAt)-new Date(item.startedAt))+'ms':'-';const summary=item.traceReplyText?summarizeAuditText(item.traceReplyText):(item.errorMessage||item.resultSummary||'运行完成');const detail=['duration='+duration,'started='+fmtTime(item.startedAt),'finished='+fmtTime(item.finishedAt),'errorCategory='+(item.errorCategory||'-'),'retryable='+(item.retryable===1?'true':item.retryable===0?'false':'-')].join('\\n');const hasTrace=Boolean(item.traceUserText||item.traceReplyText);const traceBody=hasTrace?'<div class="audit-columns">'+auditSection('任务输入（进入模型的 prompt）',item.traceUserText||'-')+auditSection('模型输出（清洗后）',item.traceReplyText||item.traceRawReplyText||'-','primary')+'</div>':(item.errorMessage?auditSection('错误',item.errorMessage,'audit-error'):'');const traceDetails=hasTrace?'<details class="audit-details"><summary>展开模型原始回复与技术字段</summary><div class="audit-columns">'+auditSection('模型原始回复',item.traceRawReplyText||'-')+auditSection('Prompt / 入站提示',item.tracePromptText||'-')+'</div></details>':'';return '<div class="audit-item"><div class="audit-rail"><div><div class="audit-time">'+esc(formatAuditTime(item.createdAt).time)+'</div><div class="audit-date">'+esc(formatAuditTime(item.createdAt).date)+'</div></div><div class="audit-status">'+badge('自动化运行','info')+badge(item.status||'-',statusKind)+'</div></div><div class="audit-main"><div class="audit-head"><div class="audit-title"><div class="audit-title-row"><strong>'+esc(item.taskName||'自动化任务')+'</strong><span class="mono">'+esc(item.taskId||'-')+'</span></div><div class="audit-summary">'+esc(summary)+'</div></div><div class="audit-meta">'+meta.map((pair)=>'<span>'+esc(pair[0]+'='+pair[1])+'</span>').join('')+'</div></div><div class="audit-section"><div class="audit-section-title">运行详情</div><div class="audit-text primary">'+esc(detail)+'</div></div>'+traceBody+renderAuditUsage(item)+traceDetails+'</div></div>';}
