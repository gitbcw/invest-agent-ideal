...
export const TOOL_SPECS: readonly ToolSpec[] = [
  ...
  },
  {
    ...
    description: "Transform a staged .xlsx workbook in the current workspace: read the input file, apply structured sheet changes (appendRows, setCells, createSheets, renameSheets, setColumnWidths, setRowHeights, mergeCells, freezePanes, autoFilters), and write a NEW output .xlsx next to it. The execution environment cannot run local scripts, so use this tool instead of any helper script when an automation task must update a bound workbook; do not treat XLSX as text. For automation tasks that only append rows to a bound workbook, prefer returning stagedOutput {operation:'appendRows', sheet, rows} instead of calling this tool — the service appends deterministically. changes values: appendRows items are {sheet?, values} where values is a 2D array (outer = row, inner = cells), e.g. {\"appendRows\":[{\"sheet\":\"行业复盘\",\"values\":[[\"2026-08-19\",1,\"…\"]]}]}. It does not commit asset versions — return the output file via stagedOutput / the automation result.",
    inputSchema: {
