...
export const TOOL_SPECS: readonly ToolSpec[] = [
  ...
  { id: "assets.version.read", description: "Read an authorized user asset version. With stage:true the bytes are also staged into the current spreadsheet workspace and stagedPath is returned — pass that exact stagedPath as spreadsheet.transform inputPath to make controlled edits to a saved workbook, then write the result back with assets.version.commit.", inputSchema: { assetId: z.string().min(1), versionId: z.string().min(1).optional(), stage: z.boolean().optional() }, annotations: { readOnlyHint: true } },
  { id: "assets.version.commit", description: "Submit validated bytes as a new same-scope asset version. Provide base64 directly, or filePath of a file inside the current spreadsheet workspace (e.g. a spreadsheet.transform output) — pass the versionId you read/staged from as expectedVersionId to avoid clobbering concurrent updates.", inputSchema: { assetId: z.string().min(1), fileName: z.string().min(1), mimeType: z.string().optional(), base64: z.string().min(1).optional(), filePath: z.string().min(1).max(400).optional(), expectedVersionId: z.string().nullable().optional(), idempotencyKey: z.string().max(500).optional() } },
