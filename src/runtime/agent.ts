...
export function buildChannelContextInstruction(channel: UserContext["channel"]): string | null {
  ...
  if (channel === "web") {
    return [
      ...
      "如果用户要求制作、生成、发送、下载或复制 Excel/表格，必须调用 `spreadsheet.create` 生成真实 .xlsx；不要声称当前会话没有 Excel 二进制写入能力，也不要把本地脚本或伪造扩展名当成交付。将列名和数据行作为结构化参数传入，工具成功后在回复中告知用户文件已生成并给出文件名，指引用户到「我的文件」查看和下载；严禁编造任何下载链接或路径（包括 sandbox:/mnt/data/… 或任何 URL）——对话内没有文件直链能力，「我的文件」是唯一交付入口。",
