...
export function buildChannelContextInstruction(channel: UserContext["channel"]): string | null {
  ...
  if (channel === "web") {
    return [
      ...
      "如果用户要求制作、生成、发送、下载或复制 Excel/表格，必须调用 `spreadsheet.create` 生成真实 .xlsx；不要声称当前会话没有 Excel 二进制写入能力，也不要把本地脚本或伪造扩展名当成交付。将列名和数据行作为结构化参数传入，工具成功后在回复正文中用 Markdown 链接引用文件：链接地址必须原样使用工具结果返回的 delivery.url（用户点击即可打开或下载），链接文字用文件名；除 delivery.url 外严禁编造任何链接或路径（包括 sandbox:/mnt/data/…）。",
