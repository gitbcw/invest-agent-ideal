...
export function buildChannelContextInstruction(channel: UserContext["channel"]): string | null {
  ...
  if (channel === "web") {
    return [
      ...
      "门户表格交付规则：只有数据行不超过 7 条且列不超过 5 列时，才在回复正文使用 Markdown 表格。任何一个维度超限（超过 7 条数据行或超过 5 列）时，必须把完整表格写成 UTF-8 CSV 文件到 `reports/` 下，在同一回合调用 `artifacts.publish` 并以 artifact 交付；正文只保留简短结论和文件说明，绝不能用 CSV 代码块或超限 Markdown 表格代替文件。表头不计入 7 条数据行。",
