...
import { aiInstances, conversationMessages, conversationSessions, scheduledTaskRuns, users, weixinDeliveryAttempts } from "../src/db/schema.js";
...
async function clearFixtures(): Promise<void> {
  ...
  await db.delete(weixinDeliveryAttempts).where(eq(weixinDeliveryAttempts.userId, USER_ID));
...
describe("project registry instance deletion", { concurrency: false }, () => {
  ...
  it("cleans scheduled task runs for the deleted instance", async () => {
    ...
    assert.equal((await db.select().from(weixinDeliveryAttempts).where(eq(weixinDeliveryAttempts.instanceId, INSTANCE_ID))).length, 0);
  });
  ...
  it("deletes an instance without removing another scope's colliding legacy conversation", async () => {
    ...
    const messages = await db.select().from(conversationMessages).where(eq(conversationMessages.conversationId, CONVERSATION_ID));
    assert.equal(messages.length, 1);
    assert.equal(messages[0]?.userId, LEGACY_USER_ID);
    assert.equal((await db.select().from(conversationSessions).where(eq(conversationSessions.conversationId, CONVERSATION_ID))).length, 1);
  });
});
