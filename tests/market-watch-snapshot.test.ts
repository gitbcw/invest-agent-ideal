...
test("market_watch.snapshot is scoped to the MCP user and instance", async () => {
  ...
  try {
    const { db, initDb } = await import("../src/db/index.js"); const { marketWatchSnapshots, sandboxAuditLogs } = await import("../src/db/schema.js"); const { callServiceTool } = await import("../src/mcp/service-tools-core.js");
