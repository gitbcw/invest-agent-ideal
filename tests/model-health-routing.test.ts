...
test("auto chain routes by health and capability with degrade hysteresis", async () => {
  ...
  try {
    ...
    // 国产链逐级降级：glm → deepseek-flash → doubao
    // （qwen3.7-flash 下线 owner 2026-09-11，qwen-3.8-next 开放后回归；
...
test("resolveAutoModel exclude honors in-turn fallback skips", async () => {
  ...
  try {
    ...
    assert.equal(resolveAutoModel({ hasImage: true, exclude: ["glm-5.3-flash", "deepseek-flash"] }).model, "doubao-seed-2-1-turbo-260628");
  } finally {
    await rm(tempRoot, { recursive: true, force: true });
    delete process.env.DB_PATH;
    delete process.env.WORKSPACE_ROOT;
...
test("model catalog keeps terra+luna from the GPT series; sol and 5.5 stay disabled (owner 2026-08-26)", async () => {
  ...
  // DeepSeek 4.1 统一合并（2026-09-10 二次裁决）：全系统只保留 deepseek-flash，
  // 旧 ID 全部出册。
  ...
  // qwen3.7-flash 下线（owner 2026-09-11）：出册不再使用，qwen-3.8-next
  // 开放后以其新 ID 回归。
  assert.equal(MODEL_DESCRIPTIONS["qwen3.7-flash"], undefined);
