...
- 变更门选择规则表的七面在盘点完成后重算覆盖率（**最终 32/30 executable，跨过最低目标线**；确定性盘点全部闭合，行为级 replay 批次一/二执行完毕：EV-001~008、EV-013 升 executable；唯一剩余 candidate 为 EV-009，挂起观察于 BC-20260824-001）。
