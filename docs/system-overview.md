...
Portal file lifecycle is service-owned and deterministic. User uploads (Portal/WeChat images and documents) keep bytes for 7 days via the authoritative `conversation_attachments.expires_at` column, then only metadata remains. AI artifacts formally published through `artifacts.publish` under `reports/**`, together with `reviews.save` output in the curated review directories, are promoted to permanent `durable_library` when their MIME is allowlisted and size is `<= 1 MiB`; supported published files are also registered in `user_assets`, so they appear in “我的文件”, and later publications of the same Workspace path append a version instead of creating a duplicate file. Oversized formal artifacts and files that do not meet these publication rules are 7-day `transient_generated`. During the internal development phase, scoped conversation artifacts may also expose the user's raw `config/**` files, including the automatically published `config/portfolio.yaml`; these files are conversation-only and do not enter the curated library. The model never decides importance — the service layer does, from source/path/size/MIME. Raw `memory/*.jsonl`, `financials/companies/**`, Skills, audit/task/alerts and the full Workspace filesystem remain outside the artifact delivery surface.
