...
第一阶段建议优先支持流式输出。如果实现成本过高，可以先做完整回复，但协议和 UI 状态应预留 `chunk` / `done` / `error` 三类事件。

流式体验要求：

- 用户发送后立即把用户消息插入消息流。
- 助手侧立即出现“正在思考”状态，避免空等。
- 收到首个 chunk 后切换为逐步展示正文。
- 结束后落定为普通助手消息。
...
- 页面能处理发送中、流式回复、失败重试和 connector 离线状态。
...
- 如果支持流式输出，助手消息按 chunk 逐步出现；如果第一版暂不支持流式，UI 仍有明确等待状态和超时提示。
...
| 密码能力做得过重 | 第一阶段复杂度失控 | 只做预置账号、登录、修改密码、退出登录；不做注册和找回密码 |

## 开放问题

1. 云端门户使用什么技术栈和部署方式？
2. 云端是否保存完整对话正文，还是只保存索引和最近 N 条？
3. 网页聊天第一版直接做 SSE 流式，还是先做完整回复但保留事件协议？
4. 密码规则第一版采用什么强度，以及是否需要管理员重置密码能力？
...
```text
请基于 docs/user-portal-design.md 推进用户门户第一阶段。不要改造 /platform 为公网用户产品，不要把云端实现成第二套 invest-agent 运行时。第一阶段用户门户必须包含账号密码登录、左侧可折叠对话历史、右侧 Chatbot 对话区、左下角用户头像菜单(仅修改密码/退出登录)、发送等待状态、失败重试和 connector 离线提示。优先实现本地 canonical conversation log 的数据模型和查询 API，然后实现 portal connector 与云端 relay，再实现网页聊天体验。所有变更必须保持微信直达 workspace ACP 主链路不变。
```

Reviewer prompt:

```text
请按 docs/user-portal-design.md 审查实现结果。重点检查：云端是否只承担 portal/relay 职责；本地 workspace/ACP/SQLite 是否仍是运行时源头；用户门户是否无法访问 Platform 管理命令；对话历史是否有 canonical log，而不是直接复用 codex_acp_traces；登录、改密、退出登录、左侧会话栏折叠、发送等待状态、流式/非流式回复状态、失败重试、connector 离线提示是否符合验收；微信链路是否无回归。
```
