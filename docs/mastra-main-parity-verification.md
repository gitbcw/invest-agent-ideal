...
| G22 | Portal 实测 | 对话内文件交付缺直链：`spreadsheet.create` 成功后助手回复中的链接是模型虚构的 `sandbox:/mnt/data/...` 格式，消息 metadata 无 artifact 附件；文件仅能从 Portal「我的文件」访问（已验证可见、可下载） | `fixed` | ✅ 已修复（2026-08-14 E5，同日按用户 H1 反馈升级）：产品语义修正——回复**应当**带可点击直链。方案：Portal 新增会话鉴权的资产字节下载路由 `/api/assets/{id}/versions/{vid}/download`（inline disposition + UTF-8 文件名）；`spreadsheet.create` 结果返回 `delivery.url` 并指示模型以 `[文件名](delivery.url)` Markdown 链接嵌入回复（仅 web 通道；微信只报文件名）；web 通道指令锚定 delivery.url 原值。实测：路由 200/正确 MIME/字节完整/未登录 401。消息内资产卡片形态仍留 G21/E9 评估 |
