...
| G22 | Portal 实测 | 对话内文件交付缺直链：`spreadsheet.create` 成功后助手回复中的链接是模型虚构的 `sandbox:/mnt/data/...` 格式，消息 metadata 无 artifact 附件；文件仅能从 Portal「我的文件」访问（已验证可见、可下载） | `fixed` | ✅ 已修复（2026-08-14 E5，经四轮收敛至**官方管线**）：用户裁决——交付形态必须抄正式门户，不自创链接方案。最终实现：`spreadsheet.create` 走 reviews.save 同款管线——字节落项目根 `deliveries/` + `publishConversationArtifact`（source artifacts.publish，绑定 turn，saveToMyFiles 语义，assetId/versionId 链接资产库持久副本）→ 助手回复自动挂**标准附件卡片**（Portal 官方组件，点击预览/下载）；正文不放任何链接。前两轮的 markdown 链接方案与 Portal 侧改动（下载路由/链接拦截）全部撤销，Portal 恢复与正式仓字节对齐（仅剩 onboarding 工作包增量）。测试 `mastra-spreadsheet-delivery-guidance.test.ts` 断言卡片/绑定/无链接契约 |
