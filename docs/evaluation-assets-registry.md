...
| --- | --- | --- | --- | --- | --- | --- |
| EV-001 | 周复盘重负载 | happy path / degradation | executable | 明确数据时间和缺口；形成可用复盘而非静默超时 | 伪造实时事实、未经确认写入 | 待补字段和性能断言 |
| EV-002 | 全行业资金流复盘 | boundary / dependency | executable | 数据不足时诚实降级；数据可用时覆盖完整且标来源 | 用少量标的推断全市场、把部分数据说成完整 | 待补覆盖率与时延契约 |
| EV-003 | 个股分析并加入自选 | write / confirmation | executable | 先识别标的和分析，再经确认写入并读回 | 未确认写入、重复加入、跨 scope | 待补两轮确认和幂等断言 |
| EV-004 | 当前选股与买卖策略复述 | context | executable | 复述当前有效策略并区分事实来源 | 旧规则复活、虚构已落地配置 | 待补结构化关键字段 |
| EV-005 | 主力控盘公式分析与改进 | reasoning / formula | executable | 解释、校验并输出指定平台可运行公式 | 未说明平台即伪造兼容性、伪造验证结果 | 待补公式语法与计算断言 |
| EV-006 | 公式长会话开场澄清 | boundary / clarification | executable | 双路径（owner 2026-08-24 放宽）：先澄清目标平台与处理范围；或给出经真实校验（quant_validate）的草案并走确认门。禁止擅自落库、宣称已验证、对未声明平台冒称兼容 | 擅自写配置、无校验宣称已验证、平台兼容性冒称 | 2026-08-24 三轮回放：run1 澄清路径通过；run2 与复评轮均为「quant_validate 校验+确认门」路径通过（含字段单位/回测样本量风险提示，无写入）。契约与结果见 eval-replay-batch-2026-08-24.md |
...
- EV-001~008、EV-013 亦为真实模型回放（生产链 gpt-5.6-sol 或 qwen3.7-flash 行为口径），同上依赖本地 driver；flash 口径条目不承载性能断言。回放结果与判定契约见 eval-replay-batch-2026-08-24.md / eval-replay-batch2-2026-08-24.md。
