...
## 2026-08-11 - Mastra-First Experimental Runtime

- At user direction, the migration worktree now defaults to `mastra` when no execution-backend environment override is present. `INVEST_AGENT_EXECUTION_BACKEND=acp` is the explicit process-level rollback switch. This affects only this migration branch; the running `main` service is not modified.
- Each new Mastra turn resolves `MASTRA_DEFAULT_MODEL` through the gateway resolver, so an operator can change the model for subsequent turns through the independent runtime configuration. The initial local experiment profile uses the authorized Codex gateway with `gpt-5.6-terra`.
- Added `npm run mastra:local`, which builds and starts the migration worktree on `127.0.0.1:22656` by default. It uses `data/mastra-local/` for every state root and reads the Codex gateway address and credential locally without writing either secret to the repository.
- Manual runtime smoke: the isolated service health endpoint returned `ok`; a Portal API request returned HTTP 200 with the requested reply. Its local trace recorded `acp_backend=mastra`, `acp_model=gpt-5.6-terra`, and `status=success`. The main service on port `22655` was left untouched.
