...
- 语义错误发现仍慢于程序错误发现；语义回归门已建立两级形态并完成首两份 SCR 实战（2026-09-06 发布），阈值校准随 9-13 观察窗口收口。
- 诊断链生产覆盖率已实测（2026-09-06 首轮：99.6% 串通）；GAP-1/GAP-2 修复已随 20260906T020157Z 发布，待 2026-09-13 复测收口；发布记录纪律已自本次恢复逐次记录。
- 评估样例、用户反馈和跨场景效果证据仍在扩充，不能把局部测试通过等同于可扩大范围（扩大范围前置条件见 governance-review-2026-09-06.md，条件①已达成）。
