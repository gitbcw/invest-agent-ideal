...
## 决策结果（用户已确认 2026-07-30）

**8 类非价格规则全部退役**（status 改 deprecated，禁止新建）。

用户理由：以后通过外部量化选股/筛选工具实现这些条件——在该工具开接口并对接，到点调用接口判断即可，不需要在本项目自建。

**price_cross 保留**（不在八类中，WP5 已迁移到窄事实接口）。
