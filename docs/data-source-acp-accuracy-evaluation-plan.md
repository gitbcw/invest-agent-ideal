...
- 正式逐例结论、证据理由、复验条件和 finding owner 见 `docs/data-source-acp-accuracy-baseline-2026-07-26.md`。阶段 4 首批工作已收敛为：历史价格精度/舍入 contract、申万官方原文读取能力、ACP model metadata 观察、慢 case 分解和公告输出压缩。
...
- 并行定向复跑发现旧 runner 的毫秒级 runId 会碰撞并覆盖报告；现已改为时间戳、进程 ID 和随机后缀组合，并加入唯一性测试。详细证据和保留 runtime 见 `docs/data-source-acp-stage4-progress-2026-07-26.md`。
