...
分支：`feat/mastra-migration`（当前生产维护分支）

> 本文描述**本分支的现实架构**。`docs/system-overview.md` 中关于旧 main/ACP runtime 的内容仅作历史对照，不是当前生产运行说明。
>
...
| D28 | **E8 执行记录**（2026-08-15）：workspace 回滚后端按 D21 时机拆除（迁移验证 + H1 双门满足后）——mastra 投影为唯一运行时后端；WORKSPACE_BACKEND env 不再影响运行时；workspace/sqlite 后端实现文件保留为迁移只读工具白名单；G17 语义随之终局（配置快照类 artifact 发布点全部清零）；测试迁移含 4 处 workspace 专属行为测试按语义改写（preferences 变更日志注入/工件强制发布两场景、legacy 编排 flag、文件浏览根迁移）。至此 E1-E10 工程队列全数收口，仅余 R1 发布决策（用户 gate） |
| D27 | **E4/P4b 执行记录**（2026-08-15）：偏好散读点与 schedulerActivation 按设计稿 §4.3 全量废除——调度权威唯一化为 typed 任务（存在=启用、pause=停用）；盯盘推送模式改读任务 delivery（wechat_on_condition→NO_PUSH）；偏好中调度字段只读留档；复盘同 tick 抑制（规则巡检 vs 简报）随跨流协调消失而移除，规则命中即推。规则巡检保留 watch 配置读取（D19 划界内，非调度语义） |
...
| E4 | P4b 偏好散读点与 schedulerActivation 废除 | D13 | ✅ 已执行（2026-08-15）：5 处散读点移除（review 偏好触发/预生成、盯盘分钟循环、推送模式偏好读）；schedulerActivation 三处门全废；presets compat 双写与 onboarding 激活写移除；复盘/盯盘唯一由 typed 任务驱动；460/460 |
...
| E8 | workspace 回滚后端拆除 | D21/§4 | ✅ 已执行（2026-08-15，D28）：ACTIVE_BACKEND 常量 mastra；约 25 文件 workspace/sqlite 臂拆除（含 3 个 lib 域后端 workspace 实现、L3 指标 fail-closed、平台路由 mastra 单路）；WorkspaceStore 仅存迁移/只读工具白名单；convergence scan 新增 E8 防回潮规则；测试套件整体切 mastra 存储（460 项 0 失败） |
