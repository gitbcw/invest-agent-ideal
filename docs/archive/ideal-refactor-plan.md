...
│    ├─ reports/*.md       ← 日/周/月复盘                   │
│    ├─ financials/        ← 公司财报                       │
│    └─ skills/overlays/   ← 用户级 skill overlay           │
│                                                           │
...
- CodeX 在用户工作目录里工作(读 AGENTS.md → 读项目级 .codex/skills → 读用户 overlay/knowledge/methods → 写 memory/*.jsonl)
...
| `src/acp/hermes-stdio-agent.ts` | 默认关闭 | 2026-06-23 范围收缩已删除;原计划仅标记 `@deprecated`,后随 WP A2 一并清理 |
...
| 层 | 职责 | 落地位置 |
| :--- | :--- | :--- |
| **协议层** | 定义规则、契约、知识 | 项目级 `.codex/skills` skeleton + 用户工作空间内 `config/*.yaml` / `knowledge/*.md` / `skills/overlays/*` |
...
| 删除 hermes-stdio-agent | `src/acp/hermes-stdio-agent.ts` | 2026-06-23 范围收缩已整文件删除;原计划仅标记 `@deprecated` |
...
**产出**:主链路对 Hermes 零依赖。
**验收**:`grep -rn "hermes" src/acp/agent.ts src/acp/prompt-context-builder.ts src/channels/weixin-mobile.ts` 仅剩注释或历史兼容字符串;`src/acp/hermes-stdio-agent.ts` 与 `/api/hermes/*` 已不存在。
