...
1. **盯盘多时段**：windows 方案（一任务多触发点）vs 多任务实例——我建议 windows（用户心智"一个盯盘"）；你定
2. **预生成保留与否**：复盘预生成暂存（为推送提速）在任务化后是否保留，还是统一即时生成（简单但推送变慢）
3. **暂停语义**：pause 任务时，错过的触发补跑吗（建议不补，记录 missed）
4. **G21 Portal 呈现范围**：任务列表+运行历史+手动触发+暂停恢复，v1 是否还要"下次运行时间"预测（建议要，成本低价值高）
