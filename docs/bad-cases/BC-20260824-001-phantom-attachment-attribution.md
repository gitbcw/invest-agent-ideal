...
是否需要用户裁决：已裁决（2026-08-24）：**暂缓修复，进入观察**——幻觉率本身不确定，须观察到足够多的例子（代价考量）再决定是否处理；触发条件=真实用户反馈同类问题，或巡检/分析显示大量出现。触发前本案保持 triaged 不升级，EV-009 维持 candidate。
回归样例：EV-009（candidate，两轮失败即为本案证据；若未来触发修复，需两轮通过方可升 executable）
复核人：owner
```
...
- **第 3 次复现（2026-08-24 晚，T-372 复评轮顺带）**：同型——「按截图识别的市值计算」「截图未包含可用资金」，数据仍为 portfolio_read 真实持仓。累计 3/3 次独立会话复现，均为同型来源冒称。继续观察，未达 owner 设定的立项触发条件（真实用户反馈或巡检大量出现）。
