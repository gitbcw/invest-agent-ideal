...
> 决策目标：在消息仍对用户有用时尽力完成生成和送达；过期后停止自动重试，不向用户暴露手动补发入口，并保留可审计记录。
...
- 用户恢复会话后，仍有效的任务由队列自动重试，不向用户提示或要求手动补发。
...
| 会话恢复 | `awaiting_user` 由用户手动取回 | 用户需要承担补发操作，且会出现重复提示 |
...
- 用户重新发起微信会话时，服务只将仍有效的 `awaiting_user` 任务重新入队；不在对话中提示、不要求用户输入补发指令。
- 重入队后的投递仍受原有退避、有效期和单用户发送节流约束；已过期或未带有效期的历史项只保留审计记录。
...
| 用户在有效期内恢复会话 | 服务自动将仍有效任务重入队，由 worker 投递；对话不显示补发提示 |
| 用户在过期后恢复会话 | 旧消息标记过期，不再发送；记录保留 |
...
其中 smoke 脚本必须使用隔离数据库和假 sender。真实火山云验收、真实微信发送和历史任务重放都需要单独明确授权。
