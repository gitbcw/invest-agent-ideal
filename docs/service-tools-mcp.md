...
- Service-owned `market.*` facade tools are retired and archived under `docs/archive/service-market-data-retirement-2026-07-31/`. ACP market/provider facts should come from the external `market-data-tool` MCP discovered through the normal MCP manifest. The former `market_watch.snapshot` reader was retired on 2026-08-28 (snapshot table frozen at 2026-07-31, the tool kept serving stale facts to intraday tasks); historical rows remain in the database for audit only, with no tool entry point.
