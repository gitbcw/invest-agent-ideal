...
- **自动化外部轨跟进**（owner 2026-08-28 裁决，T-401）：automation 通道服务轨维持授权即清单（白名单 10 个已最小），但外部 MCP 轨同样收敛为「mdt top5 常驻 + 目录 + 调度壳」。依据：8-28 盯盘零推送诊断实测 automation 轮基准输入 15.3k token 中外部工具 schema ≈7.8k（占六成；交互轮「只裁 mdt 降 10%」的结论对 automation 不成立——它的服务轨早已裁过，外部轨才是剩余主战场），且 terra 重负载首字超时与 09:55/14:30 两次零工具秒拒均与此相关。开关 `AUTOMATION_TOOL_DISCOVERY=off` 独立回退。
...
## 5.1 验收记录（2026-08-31，生产流量 3 天）

> 部署 release 20260828T072317Z-89ced6c9（2026-08-28 15:23 北京）；观测窗口 8-28 15:23 ~ 8-31 10:30，生产 runtime.db 只读查询（external_mcp_tool_calls / agent_traces）。

| 验收项 | 结果 | 证据 |
| --- | --- | --- |
| ② 工具面收敛生效 | ✅ | 部署 smoke 实测外部轨 38 工具 11,518 chars → 9 工具 1,477 chars（-87%）；parity 测试护栏。运行时无清单日志（agent_traces.tool_manifest 为 legacy 字段，mastra 轨不写），生产侧以零 TOOL_NOT_IN_CATALOG 佐证 |
| ③ 壳/目录真实使用 | ✅ | 3 天 381 次外部调用覆盖 **26 种工具**，其中 21 种非常驻（须经目录+壳），**全部成功**，含此前 30 天零调用的 get_board_fundflow_rank（第 36 个新工具经壳首次真实使用）。壳传参错误 0；仅有的 3 次失败均为常驻 get_stock_news 上游 10s 超时三连发（8-29 01:04），与两段式无关。外部失败率 0.79%（3/381）低于部署前两周 1.9%（48/2522） |
| ① token 实测 | 🟡 口径修订 | 轮级 input_tokens 为 run 内多步累加，被任务步数/上下文主导（观察期交互轮 4.2 万~158 万跨度），且样本仅 13 轮、模型混杂（terra 10/luna 3）——"同任务类型对照"在自然流量下不可构造，**"平均输入 ↓40%"按轮级口径不可判定，如实记录**。可用证据：工具面本身 -87%（硬数据）；每步结构节省 ≈10k chars ≈3.3k token（多步任务随步数放大，高 cached_read 轮部分被缓存吸收）；部署后同型短任务（4.2万~8.0万）不高于部署前（5.1万~8.7万）。干净的同任务量化对照需回放基建（mgreplay 属旧系统未迁移），列为后续观测项 |
| ④ 回退演练 | ✅ | INTERACTIVE_TOOL_DISCOVERY=off 分支核查（off → 全量服务工具+外部全量）+ tests/tool-discovery.test.ts 7/7 绿（含开关 off 与开关独立性用例）。不在生产演练（需重启，非必要不打扰生产） |
| 可得性（原方案②语料抽样） | ✅ 以真实流量替代 | 3 天 26 种工具全可达、零工具缺失 bad case，比静态语料抽样更强 |
| T-401 顺带（automation 外部轨） | ✅ | automation 轮 20 种外部工具全可达（非 top5 含 get_volume_mas×10、get_zt_pool×8 等），零缺失 |

**结论**：两段式发现 3 天真实流量零功能性回归，长尾可达性与选择准确率达标；token 收益的结构部分（工具面 -87%）成立，轮级平均降幅因口径混同不可判定。roadmap P2 度量"交互轮平均输入 ↓40%"修订为"工具面 token ↓"口径（已超额达成）+ 持续观测"工具缺失 bad case 为零"。
