...
| 分类 | 处理方式 |
| :--- | :--- |
| `one_off_analysis` | 完成本次分析，不创建长期能力。 |
...
| `system_capability_request` | 形成明确的系统实现需求，不在 Workspace 内伪造实现，也不声称已经生效。 |

混合需求必须拆分。例如“新增一个估值分析方法并每天自动执行”：估值方法可以是 Workspace 扩展；新的调度任务类型属于系统能力申请。只有服务已经支持对应任务类型时，才可以通过现有配置工具调整时间或开关。
...
缺少上述能力时，禁止通过 shell、localhost HTTP、隐藏接口、直接数据库访问或猜测 token 绕过。应明确说明缺少的能力，并生成 `system_capability_request`。

## 扩展流程

1. 检查当前 Workspace、`.codex/skills/` 和已挂载的具名 MCP 工具。
2. 判断是否只需完成一次性分析；能不持久化就不创建能力。
3. 将需求拆分并标注为四类处理结果之一。
...
```yaml
request_name: ""
classification: "system_capability_request"
user_goal: ""
...
系统能力申请只是交付给系统工程侧的需求，不是安装结果。不得因为已经写下申请、脚本、manifest 或配置文件，就告诉用户工具、定时任务或数据源已经启用。
