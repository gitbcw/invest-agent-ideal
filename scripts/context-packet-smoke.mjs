/**
 * 烟测:ContextPacket 聚合用户/实例共享上下文。
 *
 * 用法:npm run build && node scripts/context-packet-smoke.mjs
 */

import { buildContextPacket } from "../dist/acp/context-packet.js";
import { clearPendingConfirmation, registerPendingConfirmation } from "../dist/acp/pending-state.js";
import { dailyPlanBackend } from "../dist/lib/daily-plan-backend.js";
import { rememberWeixinTurn } from "../dist/lib/weixin-conversation-memory.js";

const userId = "test-context-packet";
const instanceId = "test-instance";
const userContext = {
  userId,
  instanceId,
  projectId: "invest-agent",
  projectType: "invest-agent",
  conversationId: "conv-context",
  channel: "weixin-mobile",
};

let pass = 0;
let fail = 0;

function assert(cond, label, value) {
  if (cond) {
    pass++;
    console.log(`✓ ${label}`);
  } else {
    fail++;
    console.error(`✗ ${label}`);
    if (value !== undefined) console.error(value);
  }
}

await rememberWeixinTurn(userContext, "生成日复盘", "【2026-06-24 复盘摘要】核心判断...");
await dailyPlanBackend.upsert(userId, instanceId, {
  planDate: "2026-06-24",
  generatedAt: new Date().toISOString(),
  summary: "今日复盘摘要",
  content: "# 2026-06-24 日复盘\n\n正文",
  data: { source: "context-packet-smoke" },
});
registerPendingConfirmation(userContext, {
  kind: "alert_draft",
  summary: "赛轮轮胎跌到 11.22 提醒",
  ttlMs: 60_000,
});

const packet = await buildContextPacket(userContext);

assert(packet.user.userId === userId, "包含 userId", packet.user);
assert(packet.user.instanceId === instanceId, "包含 instanceId", packet.user);
assert(packet.recentConversation.length >= 2, "包含最近对话", packet.recentConversation);
assert(packet.pendingConfirmations.length === 1, "包含待确认事项", packet.pendingConfirmations);
assert(packet.latestArtifacts[0]?.kind === "daily_review", "包含最新复盘 artifact", packet.latestArtifacts);
assert(packet.stateSummary.latestReviewDate === "2026-06-24", "状态摘要包含最新复盘日期", packet.stateSummary);
assert(packet.toolManifest.some((tool) => tool.name === "review_records.query" && tool.policy === "read"), "包含工具 manifest", packet.toolManifest);

const { db } = await import("../dist/db/index.js");
const { chatHistory, dailyPlans } = await import("../dist/db/schema.js");
const { eq } = await import("drizzle-orm");
await db.delete(chatHistory).where(eq(chatHistory.userId, userId));
await db.delete(dailyPlans).where(eq(dailyPlans.userId, userId));
clearPendingConfirmation(userContext, "alert_draft");

console.log(`\n=== 结果: ${pass} 通过 / ${fail} 失败 ===`);
process.exit(fail > 0 ? 1 : 0);
