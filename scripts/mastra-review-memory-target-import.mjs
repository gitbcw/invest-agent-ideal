...
try {
  ...
  const existingAssets = db.prepare("SELECT a.asset_id AS assetId,a.current_version_id AS currentVersionId,v.checksum,v.storage_path AS storagePath,v.idempotency_key AS idempotencyKey FROM user_assets a JOIN user_asset_versions v ON v.version_id=a.current_version_id WHERE a.user_id=? AND a.project_id=? AND a.instance_id=? AND a.asset_id LIKE 'asset\\_review\\_%' ESCAPE '\\'").all(source.userId, projectId, source.instanceId);
...
async function collectEntries() {
  ...
  for (const plan of planEntries) { const raw = await readFile(path.join(snapshotRoot, source.workspaceId, plan.relativePath)); if (sha256(raw) !== plan.sha256) throw new Error(`MASTRA_REVIEW_MEMORY_SOURCE_CHANGED: ${plan.relativePath}`); const value = parse(raw.toString("utf8")); out.push({ recordId: `daily-plan-${source.userId}-${source.instanceId}-${plan.planDate}`, recordType: "daily_plan", businessKey: plan.planDate, payloadJson: JSON.stringify(value), sourcePath: plan.relativePath, sourceLine: null, sourceChecksum: plan.sha256 }); }
  for (const memory of mapping.mapping.memory) { const raw = await readFile(path.join(snapshotRoot, source.workspaceId, memory.relativePath)); if (sha256(raw) !== memory.sha256) throw new Error(`MASTRA_REVIEW_MEMORY_SOURCE_CHANGED: ${memory.relativePath}`); const lines = raw.toString().split(/\r?\n/).filter(Boolean); lines.forEach((line, index) => { const payload = JSON.parse(line); const lineHash = sha256(line); out.push({ recordId: `memory-${source.userId}-${source.instanceId}:${memory.relativePath}:${index + 1}:${lineHash.slice(0, 16)}`, recordType: memory.disposition, businessKey: `${memory.relativePath}:${index + 1}:${lineHash}`, payloadJson: JSON.stringify(payload), sourcePath: memory.relativePath, sourceLine: index + 1, sourceChecksum: memory.sha256 }); }); }
  return out;
}
...
function versionIdFor(sourceValue, project, checksum) { return `version_review_${sha256(`${sourceValue.userId}\0${project}\0${sourceValue.instanceId}\0${checksum}`).slice(0, 24)}`; }
function assetPathFor(sourceValue, project, relativePath, checksum) { return `assets/${assetIdFor(sourceValue, project, relativePath)}/versions/${versionIdFor(sourceValue, project, checksum)}/${path.basename(relativePath)}`; }
