...
- 折叠后右侧聊天区扩展。
- 左下角固定显示用户头像入口。
- 用户头像菜单包含使用手册、修改密码和退出登录。
- 顶部栏提供低干扰的使用手册图标入口，侧栏折叠后仍可访问。

### 使用手册

- 使用手册入口位于左下角用户头像菜单和聊天页顶部栏。
- 会话列表和“新建对话”区域不增加手册入口。
- 手册支持“网页查看”和“文档查看”切换。
- 文档查看支持 PDF 与 Markdown 预览切换，并支持下载 PDF 和 MD。
- 手册内容只覆盖 Onboarding、微信端使用和 Web 端使用，不使用“手机端”概念。
- Onboarding 说明聚焦用户如何完成投资助手初始配置，并明确推荐在微信端完成。
