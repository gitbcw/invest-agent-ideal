...
- agent 行为出错时，举证责任在 context：先重构行为发生时的完整上下文（指令、注入信息、输入锚点、工具返回），错误凡能被 context 充分解释——指令与上下文矛盾、强信号压过指令、误导性输入锚点被物化——就修 context，不动模型侧。
- 修改模型侧（提示词/规则/机制/代码）的前置条件：已证明该错误不能由 context 解释。且修复从减法开始——优先移除误导信号；加规则或加机制，以减法失败且同类摩擦重复为前提。
