...
1. 读 `/api/sandbox/strategies` 看有没有 `breakout-pullback`,确认存在
2. 读行情(K 线、量比、均线)
3. 走第二道闸门(策略已被用户指定,跳过第一道)
4. 输出预案草案
5. 等用户确认
6. 确认后调 `/api/sandbox/plans/set` 落库,带 `strategyKey: "breakout-pullback"`

如果用户说"出个预案"(没指定策略),走第一道闸门,先推荐策略再起草。
